...
DEVICE_ATTR(dvfs_max_lock_begone, S_IRUGO|S_IWUSR, show_max_lock_dvfs, set_max_lock_dvfs);
DEVICE_ATTR(dvfs_min_lock_begone, S_IRUGO|S_IWUSR, show_min_lock_dvfs, set_min_lock_dvfs);
...
int gpu_create_sysfs_file(struct device *dev)
{
	...
	if (device_create_file(dev, &dev_attr_dvfs_max_lock_begone)) {
		GPU_LOG(DVFS_ERROR, DUMMY, 0u, 0u, "couldn't create sysfs file [dvfs_max_lock_begone]\n");
		goto out;
	}

	if (device_create_file(dev, &dev_attr_dvfs_min_lock_begone)) {
		GPU_LOG(DVFS_ERROR, DUMMY, 0u, 0u, "couldn't create sysfs file [dvfs_min_lock_begone]\n");
		goto out;
	}
...
void gpu_remove_sysfs_file(struct device *dev)
{
	...
	device_remove_file(dev, &dev_attr_dvfs_max_lock_begone);
	device_remove_file(dev, &dev_attr_dvfs_min_lock_begone);
